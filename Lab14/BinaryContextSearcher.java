public class BinaryContextSearcher extends ContextSearcher {
	...
	public String find(Word query, int window) {
		...
		if(useget)
		{
			return getSnippet(sortedWords.get(get+1), window);
		}
	...
	public int findHelper(Word query, int window, int first, int last) {
		...
		if(query.equals(sortedWords.get(center)))
		{
			return center;
		}
		else if(query.compareTo(sortedWords.get(center)) > 0) 
		{
			return findHelper(query, window, center+1, last);
		}
		else if(query.compareTo(sortedWords.get(center)) < 0)
		{
			return findHelper(query, window, first, center-1);
		}
		return -1;
